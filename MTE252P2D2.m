...
f_low = 100; % Hz
f_high = 7000; % less than 8000 to stay within Nyquist bounds
band_edges = linspace(f_low, f_high, N+1); % linear spacing
filter_order = 4;
...
% PHASE 3 - linear spacing
% Determine center frequencies
centers = zeros(N,1);
for i = 1:N
    f1 = filters{i}.range(1);
    f2 = filters{i}.range(2);
    centers(i) = (f1 + f2) / 2;  % arithmetic mean since bands are linearly spaced
end
...
for i = 1:N
    ...
    env = env(:);
    if length(env) > L
        env = env(1:L); % truncate if longer
    elseif length(env) < L
        env = [env; zeros(L - length(env), 1)];  % pad with zeros if shorter
    end
